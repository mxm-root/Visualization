...
% plotting
videomaker(X',Y',Z',Xc',Yc',Zc',Pi',Ro',Ya',Pic',Roc',Yac',V,F,C,V2,F2,C2,25,25,1);
...
%     scale1, scale2   normalization factor 
...
xlabel('\itx \rm(\itm\rm)'); ylabel('\ity \rm(\itm\rm)');zlabel('\itdepth \rm(\itm\rm)');
...
for i = 1:3:length(P)
    ...
    subplot(2,1,1);xlabel('\itrange \rm(\itm\rm)');ylabel('\itdepth \rm(\itm\rm)');
